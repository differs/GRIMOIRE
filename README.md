...
7. **延迟补偿与快照回放**：输入携带帧号，服务端延迟 2 帧应用（吸收抖动、同帧公平）；权威快照环形缓冲，客户端断帧后 `BATTLE_RESYNC` 重放缺失帧。demo 展示"客户端预测 vs 服务端权威"（预测领先约 2 帧后收敛）。
8. **鉴权与 TLS**：`SYS_AUTH` 握手校验 token，未鉴权连接拒绝业务请求；网关 TCP 可选 rustls TLS（`--tls-cert/--tls-key`），sim 支持 `--tls`（dev 接受自签）。
9. **跨节点撮合**：Redis 撮合大厅（SETNX/GETDEL）——服务建局登记等待局，网关对无绑定的匹配请求原子取走并路由到托管节点，战斗/牌局自动配对不再依赖已知会话 ID。
10. **会话目录失效与再平衡**：会话结束（房间/对局删除）时删除目录条目；托管节点宕机时，网关在请求路由到死节点后自动失效该会话（清绑定/缓存/目录条目），玩家可重新匹配；新节点上线后，一致性哈希环重建使新会话自然分流（懒再平衡）。
11. **会话目录 + 按会话分片路由**：Redis 存 `session(房间/对局/战斗) → 托管节点`；客户端 `SYS_BIND_SESSION` 告知网关归属会话，网关查目录把同一会话内所有玩家的请求路由到同一实例（玩家状态与房间/对局同节点，跨玩家不碎片化）。**有内存态玩法从此可以多实例水平扩容**——A 在实例 1 建房、B 在实例 2 也能加入同一房间。
...
早期 README 的"未做清单"已大部分落地，现状如下：

**已实现**
- **KCP 可靠 UDP**：网关 KCP 会话层 + `battle-kcp`/`battle-stress` 模式（0 丢帧、20Hz 稳定）
- **Postgres + Redis 持久化**：`ProfileStore` 登录建档、对局结算 UPSERT
- **网关连接迁移**：`SYS_RESUME` 断线重连无缝重绑
- **热路径锁基本消除**：session 玩法域 `AtomicU32` 位掩码、resolve 原子时间戳、会话目录本地缓存；仅 battle/card 每会话一把 `tokio::sync::Mutex`（天然串行，无需异步化）

**已实现（本轮）**
- 实时对战**延迟补偿与快照回放**（输入帧号延迟缓冲 + 权威快照环形 + BATTLE_RESYNC）
- **鉴权与 TLS**（SYS_AUTH + rustls 网关接入）
- **跨节点撮合**（Redis 撮合大厅自动配对）
- **会话目录失效与再平衡**（会话结束删条目 + 节点宕机请求触发失效自愈 + 新节点懒分流）
