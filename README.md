...
**真实吞吐上限 ≈ 200k QPS**（双网关 + 单 room-svc + etcd 全链路）。管线模式还暴露并修复了压测客户端 `Client::clone` 复制 seq 计数器导致响应串号的 bug（改为共享 `Arc<AtomicU32>`）。
...
**注意**：多实例轮询负载均衡对**有内存态**的服务（room 玩家状态、battle 战斗实例、card 对局）会破坏会话亲和——登录/建房散落到不同实例。无状态玩法（如 ROOM_LIST）没问题；有状态玩法需会话亲和（按 conn_id hash 固定节点）或共享状态层。当前 `Discovery::resolve` 是纯轮询，这是已知简化。
