...
**尚未实现**
- **实时对战延迟补偿与快照回放**：帧同步进阶——客户端本地预测 + 服务端权威帧回滚 + 断帧重放；需要帧缓存、状态快照与插值机制（可参考 `battle-svc` 的确定性模拟继续扩展）
- **鉴权与 TLS**：当前无登录校验/流量加密；可加 JWT 握手 + TLS 接入层（`grimoire-net` 的帧协议已留扩展位）
- **跨节点自动撮合**：战斗/牌局配对需撮合协调器（如 Redis 原子锁选节点），当前 demo 用已知会话 ID 演示
