...
**多实例与会话亲和**：`Discovery::resolve` 实现了**一致性哈希**（每节点 100 虚拟点，conn_id 就近路由）——同一连接的请求固定落在同一节点，修复了单连接状态的碎片化（玩家登录/建房不散落）。但**房间/战斗这类跨连接交互的服务**（一个房间里有多个玩家的连接）仍需要**共享状态层**（如 Redis 存房间数据）才能在多实例下工作——单连接亲和只保证"我自己的状态"一致，A 建房间、B（哈希到另一实例）加入不了。这是有状态服务水平扩展的根本课题。
