...
10. **会话目录 + 按会话分片路由**：Redis 存 `session(房间/对局/战斗) → 托管节点`；客户端 `SYS_BIND_SESSION` 告知网关归属会话，网关查目录把同一会话内所有玩家的请求路由到同一实例（玩家状态与房间/对局同节点，跨玩家不碎片化）。**有内存态玩法从此可以多实例水平扩容**——A 在实例 1 建房、B 在实例 2 也能加入同一房间。
...
**尚未实现**
- **会话目录失效与再平衡**：节点下线时迁移/清理会话（连接迁移已具备迁移能力，缺触发与协调）
